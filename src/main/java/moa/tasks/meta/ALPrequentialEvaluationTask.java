...
public class ALPrequentialEvaluationTask extends ALMainTask {
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        	   ...
        	   && ((maxSeconds < 0) || (secondsElapsed < maxSeconds))) 
        {
            ...
            long predictstart = TimingUtils.getNanoCPUTimeOfCurrentThread();
            double[] prediction = learner.getVotesForInstance(testInst);
            long predictend = TimingUtils.getNanoCPUTimeOfCurrentThread();
            predicttime += TimingUtils.nanoTimeToSeconds(predictend - predictstart);
            
        	evaluator.addResult(testInst, prediction);
        	
        	// train on instance
        	long trainstart = TimingUtils.getNanoCPUTimeOfCurrentThread();
            learner.trainOnInstance(trainInst);
            long trainend = TimingUtils.getNanoCPUTimeOfCurrentThread();
            traintime += TimingUtils.nanoTimeToSeconds(trainend - trainstart);
