...
public class Semisynth {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] datasets = new String[]{"CRIMES-D1", "DJ30-D1", "GAS-D1", "OLYMPIC-D1", "POKER-D1", "SENSOR-D1",
                "TAGS-D1", "ACTIVITY_RAW-D1", "ACTIVITY-D1", "CONNECT4-D1", "COVERTYPE-D1",};

        String[] algorithms = new String[]{"moa.classifiers.trees.GHVFDT", "moa.classifiers.trees.HDVFDT", "moa" +
                ".classifiers.meta.imbalanced.ROSE", "moa.classifiers.meta.OzaBagAdwin", "moa.classifiers.meta" +
                ".LeveragingBag", "moa.classifiers.meta.AdaptiveRandomForest", "moa.classifiers.meta.OOB", "moa" +
                ".classifiers.meta.UOB",};

        String[] algorithmsFilename = new String[algorithms.length];

        for (int alg = 0; alg < algorithms.length; alg++)
            algorithmsFilename[alg] = algorithms[alg].replaceAll(" ", "").replaceAll("moa.classifiers.meta" +
                    ".imbalanced" + ".", "").replaceAll("moa.classifiers.meta.", "").replaceAll("moa.classifiers" +
...
//				Utils.metric("evaluation time (cpu seconds)", "last", absolutePath+resultsPath, algorithmsFilename,
//				datasetsFilename);
//				Utils.metric("model cost (RAM-Hours)", "averaged", absolutePath+resultsPath, algorithmsFilename,
//				datasetsFilename);
            }
        }
    }
